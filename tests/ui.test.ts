...
describe('renderDemoButtons', () => {
  ...
  it('button text is the demo label and dataset has equation', () => {
...
describe('bootstrap', () => {
  ...
  it('clicking balance button shows result', () => {
    bootstrap();
    const input = document.querySelector<HTMLInputElement>('#equation-input')!;
    input.value = 'H2 + O2 -> H2O';
    document.querySelector<HTMLButtonElement>('#balance-btn')!.click();
    expect(document.querySelector('.result.ok')?.textContent).toBe('2H2 + O2 = 2H2O');
  });
  ...
  it('pressing Enter in the input triggers balance', () => {
    ...
    expect(document.querySelector('.result.ok')?.textContent).toBe(
      'CH4 + 2O2 = CO2 + 2H2O',
    );
  });
